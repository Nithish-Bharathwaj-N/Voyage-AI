...
export class ClaudeProvider implements AiProvider {
  ...
  async generateText(
    ...
    if (!this.apiKey) {
      throw new ProviderUnavailableException(this.name, 'ANTHROPIC_API_KEY is not configured');
    }
    ...
    if (!content) {
      throw new ProviderUnavailableException(this.name, 'Received empty content from Claude');
    }
