...
export class OpenRouterProvider implements AiProvider {
  ...
  async generateText(
    ...
    if (!this.apiKey) {
      throw new ProviderUnavailableException(this.name, 'OPENROUTER_API_KEY is not configured');
    }

    const model = options?.model || 'meta-llama/llama-3-8b-instruct';
    ...
    const response = await this.httpService.request<any>(this.name, url, {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://voyageai.dev',
        'X-Title': 'VoyageAI Planner',
    ...
    if (!content) {
      throw new ProviderUnavailableException(this.name, 'Received empty content from OpenRouter');
    }
