                        ...
                        <div className="space-y-2">
                          {(dest.festivals || []).map((f, i) => (
                            <div key={i} className="flex items-center gap-2 text-sm text-slate-600">
                              ...
                              <span>{typeof f === 'string' ? f : (f as any).name}</span>
                            </div>
                          ))}
                        </div>
                      </div>
            ...
            <AnimatePresence mode="popLayout">
              {[
                ...
                <motion.div 
                  ...
                  className="text-center bg-white/5 backdrop-blur-md px-6 py-4 rounded-3xl border border-white/10 hover:bg-white/10 transition-colors"
                >
                  <motion.div 
                    key={s.value} // Animate when value changes
                  ...
                  <div className="text-white/60 text-xs uppercase tracking-widest font-bold mt-1">{s.label}</div>
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-12 py-4 bg-white/10 backdrop-blur-xl rounded-2xl text-white placeholder-white/60 text-base border border-white/20 focus:outline-none focus:ring-2 focus:ring-white/50 focus:bg-white/20 transition-all shadow-2xl"
              />
              {search && (
                <button onClick={() => setSearch('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/60 hover:text-white bg-white/10 p-1 rounded-full transition-colors">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold transition-all whitespace-nowrap border ${
                    active ? `bg-gradient-to-r ${cat.color} text-white border-transparent shadow-xl ring-2 ring-indigo-500/20 scale-105`
                           : 'bg-white/60 backdrop-blur-md text-slate-600 border-slate-200/50 hover:bg-white hover:shadow-md'
                  }`}
                >
          ...
          >
            <AnimatePresence mode="popLayout">
              {filtered.map((dest, i) => (
                ...
                >
                  <DestinationCard dest={dest} onClick={() => setSelectedSlug(dest.slug)} />
                </motion.div>
              ))}
            </AnimatePresence>
          </motion.div>
