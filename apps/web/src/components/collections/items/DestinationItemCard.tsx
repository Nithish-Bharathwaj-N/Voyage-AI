...
import type { SavedDestination } from '@/lib/collections/types/collections.types';

export function DestinationItemCard({ item }: { item: SavedDestination }) {
  return (
    <Link 
    ...
    >
      <div className="relative w-24 h-24 rounded-xl overflow-hidden shrink-0">
        ...
        />
        <div className="absolute top-1 left-1 bg-black/60 backdrop-blur-md p-1 rounded-lg">
      ...
      <div className="flex flex-col justify-between py-1 flex-1 min-w-0">
        <div>
          ...
          <p className="text-sm text-muted-foreground truncate">{item.country}</p>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Destination</span>
        </div>
      </div>
    </Link>
  );
