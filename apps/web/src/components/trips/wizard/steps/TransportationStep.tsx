...
import { transportationSchema, type TransportationFormData } from '@/lib/trips/validations/wizard.schema';
...
export function TransportationStep() {
  ...
  const onSubmit = (data: TransportationFormData) => {
    updateFormData(data);
    nextStep();
  };
  ...
  const toggleOption = (id: string) => {
    if (id === 'none') {
      setValue('transportation', ['none'], { shouldValidate: true });
      return;
    }
    const current = selected.filter(s => s !== 'none');
    const next = current.includes(id) ? current.filter(i => i !== id) : [...current, id];
    setValue('transportation', next, { shouldValidate: true });
  };
