...
export function TravelStatistics({ statistics }: TravelStatisticsProps) {
  // Merge backend stats with dummy defaults for missing fields
  const statsFromStore = {
    countriesVisited: statistics?.countriesVisited || 1,
    statesVisited: statistics?.statesVisited || 4,
    citiesExplored: statistics?.citiesExplored || 12,
    totalTrips: statistics?.totalTrips || 0,
    totalBudget: statistics?.totalBudget || 45000,
    travelStreak: statistics?.travelStreak || 3,
    favoriteCategory: statistics?.favoriteCategory || 'Nature & Wildlife',
  };
